...
class BatchNormConfig:
    ...
    use_batch_statistics_training: bool = False
    detach_batch_statistics: bool = True
...
class Config:
    ...
    run_name: str = "vicreg_11_batchless"
    data_path: str = "./data"
    seed: int = 42

    batch_size: int = 1 # must be multiple of chunk_size
    chunk_size: int = 2 # size of coherent same-class chunks
    coherence: float = 1.0  # 1.0 = perfect coherence, 0.0 = random reorder

    train_encoder_concurrently: bool = False
    epochs: int = 5 # 10
    classifier_epochs: int = 8 # 20
    ...
    activation_store_momentum: float = 0.9994 # TODO
    cov_momentum: float = 0.9994
    last_z_momentum: float = 0.0

    update_last_z_before: bool = True
    update_leaky_integrators_once: bool = True
...
class LeakyIntegrator:
    value: torch.Tensor
    momentum: float
    overwrite_once: bool = True # TODO: try with False
    "Whether to overwrite the value one time. E.g. as initialization"

    def update(self, new: torch.Tensor):
        if self.overwrite_once:
            self.value = new.detach()
            self.overwrite_once = False
        else:
...
class MappingStore:
    mu: LeakyIntegrator
    var: LeakyIntegrator
    cov: LeakyIntegrator # TODO: remove
    ...
    def update(self, z: torch.Tensor):
        ...
        self.mu.update(z.mean(dim=0))
        z_centered = z - self.mu.value 
        # var = (z_centered**2).mean(dim=0)
        var = torch.var(z, dim=0, unbiased=False)
        self.var.update(var)
...
class TCLoss(nn.Module):
    ...
    def forward(self, z: torch.Tensor, lateral: nn.Module, store: MappingStore):
        ...
        sim_loss = sim_loss_pn.sum()
        std_loss = std_loss_pn.sum()
        cov_loss = cov_loss_pn.sum()
        lat_loss = lateral_loss_pn.sum()
...
class NormalizedMapping(nn.Module):
    ...
    def __init__(self, cfg: Config, in_dim, out_dim, norm):
        ...
        self.update_last_z_before = cfg.update_last_z_before

        # for batchless training
        li_m = cfg.activation_store_momentum
        self.store = MappingStore(
            mu=LeakyIntegrator(torch.zeros(out_dim), li_m, overwrite_once=cfg.update_leaky_integrators_once),
            var=LeakyIntegrator(torch.ones(out_dim), li_m, overwrite_once=cfg.update_leaky_integrators_once), 
            cov=LeakyIntegrator(torch.zeros((out_dim, out_dim)), cfg.cov_momentum, overwrite_once=cfg.update_leaky_integrators_once),
            last_z=LeakyIntegrator(torch.zeros(out_dim), cfg.last_z_momentum, overwrite_once=cfg.update_leaky_integrators_once),
            pre_nonlin_mu=LeakyIntegrator(torch.zeros(out_dim), cfg.batchnorm_config.momentum, overwrite_once=cfg.update_leaky_integrators_once),
            pre_nonlin_var=LeakyIntegrator(torch.ones(out_dim), cfg.batchnorm_config.momentum, overwrite_once=cfg.update_leaky_integrators_once),
        )
...
def build_dataloaders(cfg: Config):
    # assert cfg.batch_size < cfg.chunk_size or cfg.batch_size % cfg.chunk_size == 0, "batch_size must be a multiple of chunk_size"
