...
class Config:
    ...
    run_name: str = "vicreg_mlp_same_class_chunks_interleaved"
    data_path: str = "./data"
    seed: int = 42
...
class SameClassPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # return a stacked tensor so DataLoader collates to shape (B, 2, C, H, W)
...
class RandomTargetVarianceLoss(nn.Module):
    ...
    def forward(self, z: torch.Tensor):
        ...
        if not self.invariance_l1:
            sim_loss_per_neuron = (diff ** 2).mean(dim=0)  # MSE across pairs
        else:
            sim_loss_per_neuron = diff.abs().mean(dim=0)

        # variance penalty computed on all representations
        std_all = torch.sqrt(z.var(dim=0) + 1e-4)
        if self.bidirectional:
            raise NotImplementedError()

        std_loss_per_neuron = F.relu(self.variance_targets - std_all)

        # local loss: sim + variance penalty
        local_loss = (self.sim_coeff * sim_loss_per_neuron + self.std_coeff * std_loss_per_neuron).sum()

        # covariance penalty as before (global)
        ...
        loss_dict = {
            'loss_sim': sim_loss_per_neuron.sum().detach(),
            'loss_std': std_loss_per_neuron.sum().detach(),
            'loss_cov': cov_loss.detach(),
            # 'mu': mean_z.mean().detach(),
            'std': std_all.mean().detach(),
        }
...
class GreedyMLPEncoder(nn.Module):
    def __init__(self, layer_dims: Tuple[Tuple[int, int], ...]):
        ...
        for i, (in_dim, out_dim) in enumerate(layer_dims):
            components = [nn.Linear(in_dim, out_dim)] #, nn.BatchNorm1d(out_dim)]
            if i != 0:
                components = [nn.ReLU()] + components
...
class LayerWiseVICRegTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        interleaved_imgs = torch.cat([pairs[:, 0], pairs[:, 1]], dim=0)  # shape (2B, C, H, W)

        # flatten for MLP: (2B, D)
        inp = interleaved_imgs.contiguous().view(interleaved_imgs.size(0), -1)
...
def build_dataloaders(cfg: Config):
    ...
    train_loader = DataLoader(train_pairs, batch_size=cfg.batch_size, sampler=sampler,
