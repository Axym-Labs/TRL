...
class Config:
    ...
    sim_coeff: float = 12.0 # 25
    std_coeff: float = 25.0 # 50
    cov_coeff: float = 1000.0 # 1_000
...
class RandomTargetVarianceLoss(nn.Module):
    ...
    def cov_loss(self, z, z_centered):
        ...
        # TODO: later change this to bmm when local
        # TODO: sum across one dimension (matrix is symmetric - doesnt matter which)

        # alternative for calculating covariance
        # cov_loss_stat = 0.0
        # for i in range(len(z_centered)):
        #     outer = torch.outer(z_centered[i], z_centered[i])
        #     outer.diagonal().zero_() # remove diagonal
        #     cov_loss_stat = cov_loss_stat + outer
        ...
        # cov_stat = cov_stat.detach()
        # tmp = z_centered @ cov_stat
        # # assert False, (tmp.shape, z_centered.shape)
