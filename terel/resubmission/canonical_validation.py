"""Run a declared canonical TeReL configuration on validation data."""
...
def load_validation_plan(path):
    ...
    if not isinstance(plan, dict) or not isinstance(plan.get("candidates"), list):
        raise TypeError("validation plan must contain a candidates list")
    ...
    if None in identifiers or len(identifiers) != len(set(identifiers)):
        raise ValueError("validation candidate ids must be present and unique")
    if len(identifiers) > int(plan.get("maximum_candidates", 24)):
        raise ValueError("validation plan exceeds its maximum candidate budget")
    return plan
...
def resolve_candidate(plan, identifier):
    try:
        ...
        raise KeyError(f"unknown validation candidate: {identifier}") from error
...
def main(argv=None):
    ...
    run_candidate(
        load_validation_plan(arguments.config),
